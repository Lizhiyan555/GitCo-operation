...
def musicgen(img,text):
    # 加载 BLIP 图像描述生成模型
    blip_processor = BlipProcessor.from_pretrained(r".\blip-image-captioning-large")
    blip_model = BlipForConditionalGeneration.from_pretrained(r".\blip-image-captioning-large").to("cuda")

    # 加载音乐生成模型
    music_processor = AutoProcessor.from_pretrained(r".\musicgen")
    music_model = MusicgenForConditionalGeneration.from_pretrained(r".\musicgen")
    # 加载图像
